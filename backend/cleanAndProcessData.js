  ...
  try {
    ...
    for (const row of rawDataResult.rows) {
      ...
      try {
        ...
        // Note: This extraction logic will depend on your n8n workflow's output format
...
/**
 * Extract scan results from raw JSON data
 * @param {Object} rawData - Raw JSON data from n8n workflow
 * @returns {Array} - Extracted scan results
 */
function extractScanResults(rawData) {
  // This function will extract the relevant scan results from the raw JSON data
  // The structure depends on your n8n workflow output
  // Below is a sample implementation, adjust according to your actual data structure
  
  try {
    // Example structure (adjust based on your actual n8n output):
    // {
    //   "scan_results": [
    //     { "target": "example.com", "port": 80, "service": "http", "product": "nginx", "version": "1.18.0" },
    //     { "target": "example.com", "port": 443, "service": "https", "product": "nginx", "version": "1.18.0" }
    //   ]
    // }
